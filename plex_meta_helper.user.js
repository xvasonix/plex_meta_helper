        ...
        if (Date.now() - lastCheck > 24 * 60 * 60 * 1000) {
            ...
            if (!result.error) {
                ...
                if (needsUpdate) {
                    ...
                    if (!existingBtn) {
                        toastr.info(`새로운 업데이트(v${latestKnownVer})가 발견!<br>(상단 메뉴에서 업데이트 가능)`, "업데이트 알림", {timeOut: 8000});
    ...
    function renderDetailHtml(data, serverId, srvConfig, container) {
        ...
        if (btnRefreshMeta) {
            btnRefreshMeta.addEventListener('click', async (e) => {
                ...
                if (renderSessionAtClick !== currentRenderSession || abortDetailMetaRefresh) return;
