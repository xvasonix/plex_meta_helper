...
0.5.11 (2026-02-20)
- 상세 정보 페이지에서 `새로고침(캐시 갱신)` 버튼 이동(GUID -> 미디어 정보)
- `YAML/TMDB 반영` 알림 메시지 수정

기타 수정(2026-02-18)
- Linux용 스크립트(plexhelper.sh): TV Show 대표/시즌 페이지에서 폴더 열기시 상위 폴더 열리는 문제 수정
- 윈도우용 파일 인코딩 변경(EUC-KR)
...
2.  '외부 플레이어 재생' 및 '폴더 열기' 기능을 사용하려면, `plexplay://`, `plexfolder://` 두 주소 형식을 열 수 있도록 하는 작업이 필요합니다.(현재 문서 하단 `외부 재생/폴더 열기` 참고)
...
5. `pmh_server.py`가 항상 실행되는 환경이 필요합니다(FF 내 `Command` 이용 추천. 포트(기본: 8899, 서버 스크립트 내에서 설정 가능) 오픈 필요). Python 스크립트 내에서 Plex DB 파일 경로를 설정합니다(스크립트 실행 환경 기준 절대 경로).
...
```json
{
    ...
    "USER_TAGS": {
        "PRIORITY_GROUP": [
            ...
            { "name": "UNCEN", "pattern": "(mopa|uncen|모파)", "target": "path" }
        ],
        "INDEPENDENT": [
