...
v0.6.37 (2026-03-08)
- 설정메뉴 툴팁 추가, 수동 매칭시 GUID 표시

v0.6.36 (2026-03-07)
- `메타 리매칭` 기능 추가: 매칭 해제 후 재매칭
- 메타 업데이트 등의 변경 발생시 Plex DB 값이 즉시 업데이트 되지 않아, 최신 데이터 반영을 위해 일부 API를 서버 요청에서 Plex API 요청으로 변경.(트레이드 오프로 기존 대비 Plex API 호출 증가)
- DB 연결 로직 개선(안전성 위주)
- 기타 개선

v0.6.34 (2026-03-06)
- GUID 변경 자동 반영시 `로딩 중...` 상태에 타임아웃(8초)/수동 해제 적용
...
v0.6.32 (2026-03-04)
- 메모리 캐시/DB 로딩 로직 개선
- 최대 메모리 캐시 건수(기본값 5000건) 사용자 설정 옵션(`MAX_CACHE_SIZE`) 추가(PMH 설정 JSON)
```json
{
    "INFO": "PMH 서버 및 Plex Server 단위 설정을 입력하세요.",
    "MAX_CACHE_SIZE": 5000,
    ...
```
- 기타 개선/버그 수정
...
v0.6.31 (2026-03-04)
- UI 메뉴에 `다중경로: ON/OFF` 설정 추가
  - 쇼 목록에서 잘못 병합된 것으로 의심되는 항목 표시(경로 기준)
  - 다중 경로 발견시: `[경로수] GUID` 형태로 표시
- 기타 개선

v0.6.30 (2026-03-03)
- 목록에서 메타 잡힌 GUID에서도 클릭시 메타 새로고침 동작 추가
- Ubuntu Desktop/Windows 에서 스트리밍시 재생 기록 동기화:
  - 로컬 실행 스크립트 업데이트 필요(Windows: plexhelper.vbs / Ubuntu: plexhelper.sh)
  - Ubuntu Desktop: socat, zenity, mpv, curl 설치 필요:
    ```bash
    sudo apt update
    sudo apt install mpv socat zenity curl
    ```
  - Ubuntu Desktop에서 스트리밍 재생은 동기화를 위해 mpv 사용(폴백: smplayer)

v0.6.29 (2026-03-02)
- `미디어 정보` 새로고침 UI 로직 수정
- 목록 태그 뱃지가 Plex 기본 UI의 뱃지와 겹치지 않도록 수정
- 관리자 계정이 아닌 홈 계정 등의 사용자 전환시 홈 화면에서 횡스크롤 영역 높이 조정 안되던 문제 수정
- 기타 개선/버그 수정
...
v0.6.27 (2026-03-02)
- `미디어 정보` 새로고침을 `정보 새로고침`(DB), `메타 새로고침`(Plex)으로 구분
- 목록에서 GUID 클릭시/상세 페이지에서 메타 새로고침시 동작 세분화
  - 메타가 없는 경우: 메타 새로고침 시도. 갱신 지연시 재클릭으로 모니터링 취소 가능(갱신 여부 모니터링. 메타 갱신 기본 타임아웃 150초)
  - 메타가 있는 경우:
    - 목록: DB 데이터 조회/갱신
    - 상세: Plex에 메타 새로고침 요청
  - 친구 서버: Plex 데이터 조회/갱신
- 예고편 등의 불필요한 요소가 UI를 위해 DB 조회 재시도되던 문제 수정
- 기타 개선/버그 수정

v0.6.26 (2026-03-01)
- 쇼 페이지에서 폴더에 시즌 폴더 트리 적용: 개별 폴더열기/스캔 가능

v0.6.25 (2026-03-01)
- 목록에서 스트리밍시 외부자막 누락 문제 수정
- 스트리밍 방식을 m3u로 변경(리눅스. 팟플레이어는 자막 문제로 스크립트에서 다시 다이렉트 URL 추출 사용)
- 위 변경으로 클라이언트(OS) 헬퍼 스크립트 수정(bash/vbs)

v0.6.24 (2026-03-01)
- 서버 설정 분리(pmh_config.yaml)
- 서버 자동 업데이트 적용(server/core 분리)
- 스크립트 업데이트 클릭시 TamperMonkey 스크립트 업데이트 오픈

v0.6.19 (v2026-02-28): 스크립트
- 상세 정보 ON/OFF 기능 복원

v0.6.18 (v2026-02-28): 스크립트
- UI 수정

v0.6.17 (2026-02-28): 서버/스크립트
