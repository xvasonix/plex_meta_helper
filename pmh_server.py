...
def api_ping():
    ...
    try:
        with get_db_connection() as conn:
            pass
        return jsonify({"status": "ok", "message": "DB connected successfully"})
    except Exception as e:
...
def api_library_batch():
    ...
    try:
        with get_db_connection() as conn:
            ...
            SELECT 
                mi.id, m.width,
                (SELECT group_concat(ms.codec || '|' || IFNULL(ms.extra_data, ''), ';;') 
    ...
    try:
        with get_db_connection() as conn:
            ...
            SELECT 
                ...
                (SELECT group_concat(ms.codec || '|' || IFNULL(ms.extra_data, ''), ';;') 
                 FROM media_streams ms WHERE media_item_id = m.id AND stream_type_id = 1) as raw_stream_data,
...
if __name__ == '__main__':
    print(f">>> PMH API Server initialized and listening on port {SERVER_PORT}")
    print(f">>> Database Path: {PLEX_DB_PATH}")
